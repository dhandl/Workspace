...
preprefix = 'QCDestimation/newAntiID'
...
picklePresel = '20151116_QCDestimation_pkl'
    ...
    for htb in sorted(signalRegion[srNJet][stb]):
      ...
      for btb in btreg:
        ...
        for dP in sorted(signalRegion[srNJet][stb][htb]):
          ...
          cQCD.Draw('Lp>>QCD_antiSelection','(weight)*('+antiSelCut+')')
          cQCD.Draw('Lp>>QCD_Selection','(weight)*('+SelCut+')')
          ...
          rCSanti = getRCS(cData, antiSelCut+trigger, deltaPhiCut)
          rCSsel = getRCS(cData, SelCut+trigger, deltaPhiCut)
          ...
          if doFit:
            LpTemplates = {'DATAantiSel':histos['DATA']['antiSelection'], 'DATAsel':histos['DATA']['Selection'],\
            ...
            bins[srNJet][stb][htb][btb][dP].update(fit_QCD)
            ...
            bins[srNJet][stb][htb][btb][dP].update({'F_seltoantisel':F_ratio, 'F_seltoantisel_err':F_ratio_err})
  ...
  for stb in sorted(signalRegion[srNJet]):
    for htb in sorted(signalRegion[srNJet][stb]):
      for btb in btreg:
        for dP in sorted(signalRegion[srNJet][stb][htb]):
          deltaPhiCut = signalRegion[srNJet][stb][htb][dP]['deltaPhi']
          Fsta        = bins[(3,4)][stb][htb][(0,0)][dP]['F_seltoantisel']
          Fsta_err    = bins[(3,4)][stb][htb][(0,0)][dP]['F_seltoantisel_err']

...
picklePath = '/data/'+username+'/results2015/TTbarPolarizationEstimation/'
picklePresel = '20151104_ttjetsPolSys_pkl'
...
def getMatch(genLep,recoLep):
  #return ( (genLep['charge']==recoLep['charge']) and deltaR(genLep,recoLep)<0.4 and (abs(genLep['pt']-recoLep['pt'])/genLep['pt'])<0.5)
  return (genLep['charge']==recoLep['charge'])
...
#        {'name':'TTJets_LO_25ns', 'sample':TTJets_HTLO_25ns, 'legendName':'t #bar{t}+Jets', 'color':ROOT.kRed, 'merge':'EWK', 'chain':getChain(TTJets_HTLO_25ns,histname=''), 'weight':'weight'},
  ...
  for stb in signalRegion[srNJet]:
    ...
    for htb in signalRegion[srNJet][stb]:
      ...
      for btb in btreg:
        ...
        for sample in Bkg:
          cutname = nameAndCut(stb, htb, srNJet, btb, presel=presel, btagVar = 'nBJetMediumCSV30')[0]
...
#          cut = '(Sum$(Jet_pt)>400&&(LepGood_pt[0]+met_pt)>200&&Sum$(LepGood_pt>25&&abs(LepGood_eta)<2.5)==1)'
...
#              print [getObjDict(sample['chain'], 'GenPart_', ['pt','eta','phi','mass','pdgId','motherId','motherIndex'], j) for j in range(int(sample['chain'].GetLeaf('nGenPart').GetValue()))]
            ...
            #find gen W
            p4t, p4w, p4lepton = getGenTopWLepton(sample['chain'])
            weight = 1.
...
#            if sample['chain'].GetLeaf('LepGood_pdgId').GetValue(0)     
            ...
            normWeight = weight * (1. - 0.05*(1.-cosTheta)**2) * 1./(1.-0.05*2./3.)
            yNorm += normWeight
